...
import { dailyRoutineQuestions, getAdaptiveDifficulty } from '../../../data/seed';
...
export default function RoutineRecallGame() {
  ...
  const questions = dailyRoutineQuestions[difficulty];
  const currentQuestion = questions[currentIndex];

  useEffect(() => { loadDifficulty(); }, []);
  useEffect(() => {
    if (currentQuestion) {
      setShuffledChoices([...currentQuestion.options].sort(() => Math.random() - 0.5));
    }
  ...
  const loadDifficulty = async () => {
    ...
    const sessions = await db.cognitiveSessions
      ...
      .and(s => s.activityType === 'daily_routine_recall')
      .toArray();
  ...
  const saveResult = async (finalScore: number) => {
    ...
    await db.cognitiveSessions.add({
      ...
      activityType: 'daily_routine_recall',
      difficulty,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-emerald-50 to-cream-100 p-4">
      <div className="flex items-center justify-between mb-6">
        <button onClick={() => navigate('/app/brain')} className="p-2 hover:bg-white/50 rounded-xl">
          <ArrowLeft size={24} />
        </button>
        <h1 className="text-lg font-bold text-gray-800">{t('routineRecall')}</h1>
      ...
      {gameComplete ? (
        <div className="text-center py-12 slide-up">
          ...
          <div className="grid grid-cols-3 gap-4 max-w-sm mx-auto mb-8">
            ...
            <div className="card-elderly text-center p-4">
              <Target className="mx-auto text-emerald-500 mb-1" size={24} />
              <div className="text-2xl font-bold">{Math.round((score / questions.length) * 100)}%</div>
        ...
        <div className="text-center">
          <div className="card-elderly p-8 mb-6 max-w-md mx-auto">
            ...
            <p className="text-elderly-lg font-semibold text-gray-800">{currentQuestion.question}</p>
          </div>
          ...
          <div className="space-y-3 max-w-md mx-auto">
            {shuffledChoices.map((choice, i) => (
              <button
                ...
                className={`w-full p-5 rounded-2xl text-left text-elderly-base font-medium transition-all active:scale-[0.98] ${
                  selectedAnswer === choice
                    ? isCorrect
                      ? 'bg-emerald-500 text-white ring-4 ring-emerald-300'
                      : 'bg-red-500 text-white ring-4 ring-red-300'
                    : choice === currentQuestion.correctAnswer && selectedAnswer
